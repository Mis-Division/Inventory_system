<template>
  <div class="main-container">
    <div >
      <h1>Category</h1>
      <p>This is a Categories</p>
    </div>
    <button :disabled="!cadAddCategory" class="btn btn-primary">
      <i class="bi bi-plus-circle me-1"></i> Add
    </button>
    &nbsp;
    <button :disabled="!canEditCategory" class="btn btn-warning">
      <i class="bi bi-pencil me-1"></i> Edit
    </button>
    &nbsp;
    <button class="btn btn-danger" :disabled="!canDeleteCategory">
      <i class="bi bi-trash me-1"></i> Delete
    </button>

  </div>
</template>
<script setup>
import { computed } from "vue";
//import { useAppStore } from "../../stores/appStore";
import { userStore } from "../../stores/userStore";
import "../../assets/css/Global.css";


//Permissions for Special Hardware
const cadAddCategory = computed(() => userStore.cadAddCategory);
const canEditCategory = computed(() => userStore.canEditCategory);
const canDeleteCategory = computed(() => userStore.canDeleteCategory);

</script>

